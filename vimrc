if !exists("g:vimstuff_sourced")
	...
	function! AddInclude(inc)
		...
		let insert_type = 0
		if includes_group != -1
			let l = search('#include <\('.g:include_priorities[includes_group].'\)', 'bW')
			if l == 0
				for i in range(includes_group + 1, len(g:include_priorities) - 1)
					normal gg
					let l = search('#include <\('.g:include_priorities[i].'\)', 'W')
					if l != 0
						let insert_type = 1
						break
					end
			...
			if l == 0
				for i in range(0, includes_group - 1)
					normal G
					let l = search('#include <\('.g:include_priorities[i].'\)', 'bW')
					if l != 0
						let insert_type = -1
						break
					end
